...
def test_load_employees():
    ...
    assert len(secret_santa.team_members) > 0
...
def test_assign_secret_santa():
    ...
    assert len(assignments) == len(secret_santa.team_members)
    assigned_children = set(child[3] for child in assignments)
    assert len(assigned_children) == len(secret_santa.team_members)  # Ensure unique assignments
...
def test_no_repeated_assignments():
    ...
    for emp_name, emp_email, child_name, child_email in assignments:
        assert child_email != secret_santa.past_matches.get(emp_email, None)
...
def test_each_employee_has_one_secret_child():
    ...
    assert len(assigned_employees) == len(assignments)
